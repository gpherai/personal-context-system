...
This is the original brainstorm list, not the current source of truth. Current accepted technology decisions are in `docs/DECISIONS.md`.

Likely stack at brainstorm time:
...
These questions were open during initial brainstorming. Resolved ones reference where the decision was recorded.

- ~~Exact project name and visual identity.~~ Resolved: `personal-context-system` / Personal Context System. See `docs/DECISIONS.md`.
- ~~Whether to start as webapp only or also include a CLI package from day one.~~ Resolved: web application first; CLI will be an adapter over existing services. See `docs/DECISIONS.md`.
- ~~Whether the generated context mirror should be markdown only, JSON only, or both.~~ Resolved: both Markdown and JSON where useful. See `docs/AI-CONTEXT.md` and `docs/DECISIONS.md`.
- ~~How typed should entries be at the database level versus flexible metadata.~~ Resolved: relational columns for stable queried fields; JSONB `metadata` for flexible type-specific context. See `docs/DATA-MODEL.md`.
- ~~How to model links/relationships generically without creating a confusing graph too early.~~ Resolved: explicit `Relationship` table with typed endpoints and relation types; graph/map view deferred. See `docs/DATA-MODEL.md`.
- ~~How to keep `detective` external-world data separate while allowing future integration.~~ Resolved: distinct canonical systems; this system stores only Gerald's personal reflections and links. See `docs/ARCHITECTURE.md`.
- ~~Which UI mode is the first primary surface: dashboard, inbox, cabinet, or ledger.~~ Resolved: Dashboard, Capture, Ledger, and Entry detail as first build surfaces. See `docs/DECISIONS.md`.
- Whether to include authentication at all for a local single-user app. **Still open.** See `docs/DECISIONS.md`.
- ~~Deployment model: local dev server, Docker, or production-style local service.~~ Resolved: Docker Compose for local PostgreSQL; Next.js dev server for the app. See `docs/LOCAL-OPERATION.md`.

## Canonical Architecture

The refined architecture has been moved to `docs/ARCHITECTURE.md`.

This file is the raw product and architecture brainstorm: original concepts, constraints, vibes, and historical questions. Use `docs/ARCHITECTURE.md` for the current buildable architecture, and `docs/DECISIONS.md` for accepted decisions and remaining open questions. Do not edit this file to track current state.
